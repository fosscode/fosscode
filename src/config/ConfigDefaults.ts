  ...
  static getDefaultConfig(): AppConfig {
    return {
      ...
      providers: {
        openai: { verbose: true },
        grok: { verbose: true },
        lmstudio: { baseURL: 'http://localhost:1234', verbose: true },
        openrouter: { verbose: true },
        sonicfree: { baseURL: 'https://gateway.opencode.ai/v1', verbose: true },
        mcp: { mcpServerCommand: 'npx', mcpServerArgs: ['@playwright/mcp@latest'], verbose: true },
        anthropic: { verbose: true },
        mock: { verbose: true },
      },
      cachedModels: {
