...
export class MCPRPCManager {
  ...
  async sendRequest(
    request: MCPRequest,
    sendMessage: (message: MCPRequest | MCPNotification) => void
  ): Promise<any> {
  ...
  setupMessageHandler(connectionManager: {
    ...
    connectionManager.setupMessageHandler(onData, onError);
  }
}
