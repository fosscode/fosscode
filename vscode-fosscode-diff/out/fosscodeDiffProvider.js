...
class FosscodeDiffProvider {
    ...
    registerContentProvider() {
        this.originalContentProviderDisposable = vscode.workspace.registerTextDocumentContentProvider('fosscode-original', this.originalContentProvider);
    ...
    handleFosscodeExecution(event) {
        vscode.window.onDidEndTerminalShellExecution(endEvent => {
    ...
    async processDiffFiles() {
        ...
        for (const [fileName, fileType] of files) {
            if (fileName.startsWith('fosscode-') &&
    ...
    async showDiff(diffData) {
        ...
        if (autoShowDiffs) {
            await vscode.commands.executeCommand('vscode.diff', originalUri, fileUri, `Fosscode Edit: ${originalFileName}`);
        }
        // Show notification
        if (showNotifications) {
            ...
            const result = await vscode.window.showInformationMessage(message, showDiffButton, dismissButton);
    ...
    async storeOriginalContent(fileName, content) {
        ...
        await vscode.workspace.fs.writeFile(vscode.Uri.file(originalPath), Buffer.from(content, 'utf8'));
    }
    getTempDir() {
        const config = vscode.workspace.getConfiguration('fosscode-diff');
